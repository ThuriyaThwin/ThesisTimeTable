 ...
 * <pre>
 * <code>
 * function BACKTRACKING-SEARCH(service) returns a solution, or failure
 *    return BACKTRACK({ }, service)
 *
 * function BACKTRACK(assignment, service) returns a solution, or failure
 *    if assignment is complete then return assignment
 *    var = SELECT-UNASSIGNED-VARIABLE(service)
 *    for each value in ORDER-DOMAIN-VALUES(var, assignment, service) do
 *       if value is consistent with assignment then
 *          add {var = value} to assignment
 *          inferences = INFERENCE(service, var, value)
 *          if inferences != failure then
 *             add inferences to assignment
 *             result = BACKTRACK(assignment, service)
...
public class BacktrackingStrategy extends SolutionStrategy {
    ...
    public int getNumberOfConsistencyCheck(){return numberOfConsistencyCheck;}
}
