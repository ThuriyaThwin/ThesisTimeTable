...
import service.csp.domain.DomainRestoreInfo;
 ...
 * <pre>
 * <code>
 * function AC-3(service) returns false if an inconsistency is found and true otherwise
 *    inputs: service, a binary CSP with components (X, D, C)
 *    local variables: queue, a queue of arcs, initially all the arcs in service
 *    while queue is not empty do
 *       (Xi, Xj) = REMOVE-FIRST(queue)
 *       if REVISE(service, Xi, Xj) then
 *          if size of Di = 0 then return false
 *             for each Xk in Xi.NEIGHBORS - {Xj} do
 *                add (Xk, Xi) to queue
 *    return true
 * 
 * function REVISE(service, Xi, Xj) returns true iff we revise the domain of Xi
 *    revised = false
 *    for each x in Di do
...
public class AC3Strategy {
	...
	/*testing MAC-3 algorithm */
	public DomainRestoreInfo reduceDomains(Variable var, Object value, CSP csp,Assignment assignment) {
		DomainRestoreInfo result = new DomainRestoreInfo();
		Domain domain = csp.getDomain(var);
		if (domain.contains(value)) {
			if (domain.size() > 1) {
	...
	private void reduceDomains(FIFOQueue<Variable> queue, CSP csp,
							   DomainRestoreInfo info, Assignment assignment) {
		while (!queue.isEmpty()) {
			Variable var = queue.pop();
			for (Constraint constraint : csp.getConstraints(var)) {
				if (constraint.getScope().size() == 2) {
