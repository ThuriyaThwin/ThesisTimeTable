 ...
 * <pre>
 * <code>
 * function MIN-CONFLICTS(service, max-steps) returns a solution or failure
 *    inputs: service, a constraint satisfaction problem
 *            max-steps, the number of steps allowed before giving up
 *    current = an initial complete assignment for service
 *    for i = 1 to max steps do
 *       if current is a solution for service then return current
 *       var = a randomly chosen conflicted variable from service.VARIABLES
 *       value = the value v for var that minimizes CONFLICTS(var, v, current, service)
